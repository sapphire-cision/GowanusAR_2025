...
{
	[CreateAssetMenu(fileName = nameof(AlphaCenterMeshAnimation), menuName = "Audio Text Synchronizer/Text Effects/Mesh/Mesh Animations/" + nameof(AlphaCenterMeshAnimation))]
	public class AlphaCenterMeshAnimation : MeshAnimationBase
	{
		...
		public override void ProcessVertexEffect(TMP_TextInfo textInfo, int startVertexIndex, int vertexIndex, float progress)
		{
			...
			textInfo.meshInfo[0].vertices[vertexIndex] = vertexStart + center - (center * ScaleRatio + center * progress * (1f - ScaleRatio));
		...
		public override UIVertex ProcessVertexEffect(UIVertex vertex, int vertexIndex, float progress)
		{
			if (verticesCenter.Count > vertexIndex / 6)
			{
				...
				var start = verticesStart[vertexIndex];
				vertex.position = start + center - (center * ScaleRatio + center * progress * (1f - ScaleRatio));
				var alpha = UseAlpha ? (byte)Mathf.Clamp(startColor.a * AlphaCurve.Evaluate(progress), 0, 255) : (byte)255;
